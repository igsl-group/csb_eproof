...
public class CertController {
    ...
    public Result searchCert(@RequestBody CertSearchDto request, @Schema(
            type = "string",
            allowableValues = {"IMPORTED", "GENERATED", "SIGN_ISSUE", "NOTIFY",
                    "ANY", "VALID", "INVALID"}) @PathVariable String searchType,
        ...
        List<String> certStageList =
                List.of(CertStage.IMPORTED.name(), CertStage.GENERATED.name(),
                        CertStage.SIGN_ISSUE.name(), CertStage.NOTIFY.name(),
                        CertStage.COMPLETED.name(), CertStage.VOIDED.name());
        List<String> certStatusList = List.of(CertStatus.PENDING.name(),
                CertStatus.SUCCESS.name(), CertStatus.IN_PROGRESS.name(),
                CertStatus.FAILED.name(), CertStatus.SCHEDULED.name());
        switch (searchType) {
            case "ANY" -> {
        ...
        Page<CertInfo> searchResult = certInfoService.search(request,
                certStageList, certStatusList, pageable);

        List<CertInfoDto> resultList =
                CertInfoMapper.INSTANCE.toDtoList(searchResult.getContent());
        /*
         * resultList.forEach(x->{ if(Objects.nonNull(x.getCertEproof())){ try { String encodedToken = URLEncoder.encode(x.getCertEproof().getToken(), StandardCharsets.UTF_8.name());
         * x.setUrl(x.getCertEproof().getUrl()+encodedToken); x.setCertEproof(null); } catch (UnsupportedEncodingException e) { e.printStackTrace(); } }
         * 
         * });
         */
        Page<CertInfoDto> returnResult = new PageImpl<>(resultList, pageable,
    ...
    @PostMapping(consumes = MediaType.MULTIPART_FORM_DATA_VALUE,
            value = "/batch/import/{examProfileSerialNo}")
    @Transactional(rollbackFor = Exception.class)
    public Result batchImport(@PathVariable String examProfileSerialNo,
            @RequestPart("file") MultipartFile file) {
        List<CertImportDto> csvData =
                CsvUtil.getCsvData(file, CertImportDto.class);
        return Result.success(
                certInfoService.batchImport(examProfileSerialNo, csvData));
    }

    @PostMapping("/batch/dispatch/{examProfileSerialNo}")
    @Transactional(rollbackFor = Exception.class)
    public Result<Boolean> dispatch(@PathVariable String examProfileSerialNo,
            @RequestParam CertStage currentStage) {
        return Result.success(
                certInfoService.dispatch(examProfileSerialNo, currentStage));
    }

    @PostMapping("/batch/scheduleMail/{examProfileSerialNo}")
    @Transactional(rollbackFor = Exception.class)
    public Result batchScheduleMail(@PathVariable String examProfileSerialNo,
            @RequestBody InsertGcisBatchEmailDto insertGcisBatchEmailDto) {
        certInfoService.changeCertStatusToScheduled(examProfileSerialNo,
                CertStage.NOTIFY);
        certInfoService.insertGcisBatchEmail(examProfileSerialNo,
                insertGcisBatchEmailDto);
        return Result.success();
    }
    ...
    public Result batchGeneratePdf(@PathVariable String examProfileSerialNo)
            throws Exception {
        certInfoService.changeCertStatusToInProgress(examProfileSerialNo,
                CertStage.GENERATED);
    ...
    /*
     * @PostMapping("/batch/signAndIssue/{examProfileSerialNo}") public Result signAndIssue(@PathVariable String examProfileSerialNo){
     * certInfoService.changeCertStatusToInProgress(examProfileSerialNo,CertStage.SIGN_ISSUE); certInfoService.batchSignAndIssue(examProfileSerialNo); return Result.success(); }
     */
    ...
    public Result updatePersonalParticular(
            @RequestBody UpdatePersonalDto personalDto) {
        return Result
                .success(certInfoService.updatePersonalParticular(personalDto));
    }

    @PostMapping("/single/updatePersonalParticular/{certInfoId}")
    public Result updatePersonalParticular(@PathVariable Long certInfoId,
            @RequestBody UpdatePersonalDto personalDto) {
    ...
    public Result updateResult(@PathVariable Long certInfoId,
            @RequestBody UpdateResultDto resultDto) {
        return Result
                .success(certInfoService.updateResult(certInfoId, resultDto));
    }

    @PostMapping("/hold/{certInfoId}")
    public Result hold(@PathVariable Long certInfoId,
            @RequestBody CertIntoUpHoldDto dto) {
    ...
    public ResponseEntity downloadPdf(@RequestParam List<Long> certInfoIdList)
            throws IOException {
        ...
        byte[] zippedPdfListByteArray =
                certInfoService.getZippedPdfBinary(certInfoIdList);
    ...
    public ResponseEntity getUnsignedJson(@PathVariable Long certInfoId) {
        ...
        return ResponseEntity
                .ok(certInfoService.prepareEproofUnsignJson(certInfoId));
    }

    @PostMapping("/eproof/prepareEproofPdf/{certInfoId}")
    public ResponseEntity prepareEproofPdf(@PathVariable Long certInfoId,
            @RequestBody PrepareEproofPdfRequest prepareEproofPdfRequest)
            throws Exception {

        byte[] preparedEproofPdf = certInfoService.prepareEproofPdf(certInfoId,
                prepareEproofPdfRequest);
    ...
    public Result startScheduledSign(@PathVariable String examProfileSerialNo) {
        ...
        List<CertInfo> scheduledCert = certInfoService
                .batchScheduleCertSignAndIssue(examProfileSerialNo);
        if (scheduledCert != null && !scheduledCert.isEmpty()) {
            return Result.success("Scheduled sign and issue started for "
                    + scheduledCert.size() + " cert(s).");
        } else {
            return Result.success(
                    "Already scheduled certs found. No new cert scheduled for sign and issue.");
        }
    }
    ...
    public ResponseEntity getNextScheduledSignAndIssueCert(
            @PathVariable String examProfileSerialNo) {

        CertInfo nextScheduledCertForProcessing = certInfoService
                .getNextScheduledSignAndIssueCert(examProfileSerialNo);
        if (nextScheduledCertForProcessing != null) {
            return ResponseEntity.ok(nextScheduledCertForProcessing.getId());
        } else {
            return ResponseEntity.badRequest()
                    .body("No next scheduled cert found.");
        }
    }

    @PostMapping("/uploadSignedPdf/{certInfoId}")
    public Result uploadSignedPdf(@PathVariable Long certInfoId,
    ...
    @PostMapping("/revoke")
    public Result revoke(@RequestParam List<Long> certInfoIdList,
            @RequestBody CertRevokeDto params) {
        certInfoRenewService.revoke(certInfoIdList, params);
        return Result.success();
    }

    @GetMapping("/approveRevoke/{certActionId}")
    public Result approveRevoke(@PathVariable Long certActionId)
            throws Exception {
    ...
    @PostMapping(consumes = MediaType.MULTIPART_FORM_DATA_VALUE,
            value = "/enquiryResult/csv")
    public ResponseEntity<byte[]> enquiryResult(
            @RequestPart("file") MultipartFile file) {
        List<EnquiryResultDto> csvData =
                CsvUtil.getCsvData(file, EnquiryResultDto.class);
        if (CollUtil.isEmpty(csvData)) {
            throw new GenericException(ExceptionEnums.UPLOAD_FLIE_IS_EMPTY);
        }
        List<String> params = csvData.stream().map(EnquiryResultDto::getNumber)
                .collect(Collectors.toList());
        return certInfoService.enquiryResult(params);
    }

    @GetMapping(value = "/sendEmail")
    public Result<Boolean> sendEmail() throws MessagingException{
        return Result.success();
    }
