...
import com.hkgov.csb.eproof.config.MinioProperty;
...
public class MinioUtil {
    ...
    public void uploadFile(String fileName, MultipartFile multipartFile) {
        minioClient.putObject(
                PutObjectArgs
                        .builder()
                        .bucket(minioProperty.getBucket()).object(fileName)
    ...
    public ByteArrayResource getFile(String path) {
        ByteArrayResource file = new ByteArrayResource(IOUtils.toByteArray(minioClient.getObject(GetObjectArgs.builder().bucket(minioProperty.getBucket()).object(path).build())));
        return file;
    }
    ...
    public InputStream getFileAsStream(String path) {
        try {
            ...
            throw new CommonException(RspMessage.FAILED_TO_READ_FILE_TYPE, e);
        }
    }
}