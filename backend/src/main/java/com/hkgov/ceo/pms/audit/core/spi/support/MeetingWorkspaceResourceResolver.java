...
/**
 * Implementation of {@link AuditResourceResolver} that show information in format {message: %s}.
 */
@Component
public class MeetingWorkspaceResourceResolver implements AuditResourceResolver {

    @Override
    public String[] resolveFrom(final JoinPoint target, final Object returnValue, final Audit audit) {
        String value = NULL_STRING;
        if (returnValue instanceof MeetingWorkspaceDto meetingWorkspaceDto) {
            value = String.format(audit.resourceWording(), meetingWorkspaceDto.getTitle());
        }
