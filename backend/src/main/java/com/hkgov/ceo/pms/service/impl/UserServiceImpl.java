...
import com.hkgov.ceo.pms.dao.PasswordResetTokenRepository;
import com.hkgov.ceo.pms.dao.UserHasGroupRepository;
import com.hkgov.ceo.pms.dao.UserHasRoleRepository;
...
import com.hkgov.ceo.pms.event.EmailEventPublisher;
...
import static com.hkgov.ceo.pms.config.Constants.ACTIVE;
import static com.hkgov.ceo.pms.config.Constants.BLOCKED;
import static com.hkgov.ceo.pms.config.Constants.DATE_PATTERN;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.DUPLICATE_EMAIL_EXCEPTION_CODE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.DUPLICATE_EMAIL_EXCEPTION_MESSAGE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.DUPLICATE_LOGIN_ID_EXCEPTION_CODE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.DUPLICATE_LOGIN_ID_EXCEPTION_MESSAGE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.OLD_PASSWORD_NOT_VALID_EXCEPTION_CODE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.OLD_PASSWORD_NOT_VALID_EXCEPTION_MESSAGE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.PASSWORD_USED_BEFORE_EXCEPTION_CODE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.PASSWORD_USED_BEFORE_EXCEPTION_MESSAGE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.RESET_PASSWORD_TOKEN_INVALID_EXCEPTION_CODE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.RESET_PASSWORD_TOKEN_INVALID_EXCEPTION_MESSAGE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.USER_CANNOT_DELETE_ITSELF_EXCEPTION_CODE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.USER_CANNOT_DELETE_ITSELF_EXCEPTION_MESSAGE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.USER_CANNOT_EDIT_SELF_STATUS_EXCEPTION_CODE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.USER_CANNOT_EDIT_SELF_STATUS_EXCEPTION_MESSAGE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.USER_EMAIL_NOT_FOUND_EXCEPTION_CODE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.USER_EMAIL_NOT_FOUND_EXCEPTION_MESSAGE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.USER_ID_NOT_FOUND_EXCEPTION_CODE;
import static com.hkgov.ceo.pms.exception.ExceptionConstants.USER_ID_NOT_FOUND_EXCEPTION_MESSAGE;
...
public class UserServiceImpl implements UserService {
    ...
    public User getUserByLoginId(String loginId) {
        return userRepository.findByLoginId(loginId)
                .orElseThrow(() -> GenericException.Builder
                        .create()
                        .setCode(USER_ID_NOT_FOUND_EXCEPTION_CODE)
                        .setMessage(USER_ID_NOT_FOUND_EXCEPTION_MESSAGE)
                        .setValue(loginId)
                        .build());
    }
    ...
    public User getUserByEmail(String email) {
        return userRepository.findByEmail(email)
                .orElseThrow(() -> GenericException.Builder
                        .create()
                        .setCode(USER_EMAIL_NOT_FOUND_EXCEPTION_CODE)
                        .setMessage(USER_EMAIL_NOT_FOUND_EXCEPTION_MESSAGE)
                        .setValue(email)
                        .build());
    }
    ...
    private void validateEmailDuplication(UserDto request) {
        if (StringUtils.isNotEmpty(request.getEmail()) && userRepository.existsByEmail(request.getEmail())) {
            throw new GenericException(DUPLICATE_EMAIL_EXCEPTION_CODE, DUPLICATE_EMAIL_EXCEPTION_MESSAGE);
        }
    }
    ...
    private void validateLoginIdDuplication(UserDto request) {
        if (userRepository.existsByLoginId(request.getLoginId())) {
            throw new GenericException(DUPLICATE_LOGIN_ID_EXCEPTION_CODE, DUPLICATE_LOGIN_ID_EXCEPTION_MESSAGE);
        }
    }
    ...
    public User updateUser(UserDto request) {
        ...
        if (isCurrentUser(user) && !ACTIVE.equals(request.getStatus())) {
            throw new GenericException(USER_CANNOT_EDIT_SELF_STATUS_EXCEPTION_CODE, USER_CANNOT_EDIT_SELF_STATUS_EXCEPTION_MESSAGE);
        }
    ...
    private void validateOldPassword(String oldPassword, User user) {
        if (!passwordEncoder.matches(oldPassword, user.getPassword())) {
            throw new GenericException(OLD_PASSWORD_NOT_VALID_EXCEPTION_CODE, OLD_PASSWORD_NOT_VALID_EXCEPTION_MESSAGE);
        }
    }
    ...
    public void validatePasswordResetToken(String token) {
        ...
        if (isNull(passToken) || passToken.isUsed() || isTokenExpired(passToken)) {
            throw new GenericException(RESET_PASSWORD_TOKEN_INVALID_EXCEPTION_CODE, RESET_PASSWORD_TOKEN_INVALID_EXCEPTION_MESSAGE);
        }
    }
    ...
    private void validatePasswordHistory(User user, String encodedPassword) {
        user.getPasswords()
                ...
                .findAny()
                .ifPresent(p -> {
                    throw new GenericException(PASSWORD_USED_BEFORE_EXCEPTION_CODE, PASSWORD_USED_BEFORE_EXCEPTION_MESSAGE);
                });
    }
